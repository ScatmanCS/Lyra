  ...
  implements CommandExecutor
{
  ...
  public Command_notes()
  {
    ...
    this.data = YamlConfiguration.loadConfiguration(new File(Lyra.getInstance().getDataFolder().getAbsolutePath(), "notes.yml"));
    ...
    if (args.length != 1)
    {
      if ((args.length == 2) && (args[0].equalsIgnoreCase("check")))
      {
        ...
        try
        {
          ...
          {
            p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&2" + args[1] + " &ccould not be found"));
            return true;
          }
          if (this.data.getInt(response + ".Amount Of Notes") == 0)
          {
            p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&2" + args[1] + " &cdoes not have any &6notes&c!"));
            return true;
          }
          ...
          for (int i = 0; i < notes.length; i++)
          {
            ...
            p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&6[&e" + i + "&6] " + this.data.getString(new StringBuilder(String.valueOf(String.valueOf(p.getName()))).append(".Last_Note_Date").toString()) + " &7" + notes[i]));
          }
        }
        catch (Exception e)
        {
      ...
      if (args[0].equalsIgnoreCase("clearall"))
      {
        ...
        try
        {
          ...
          {
            this.data.set(response + ".Amount Of Notes", Integer.valueOf(this.data.getInt(response + ".Amount Of Notes") - 1));
            this.data.save(this.notesFile);
            clearList.remove(j);j--;
          }
      ...
      if (args[0].equalsIgnoreCase("remove"))
      {
        ...
        try
        {
          ...
          p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cUsage&7: /note remove [player] [note number]"));
          return true;
        }
        catch (Exception e)
        {
      ...
      if (args.length >= 3)
      {
        ...
        if (args[0].equalsIgnoreCase("add"))
        {
          ...
          p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&eAdded a &6note&e to &2" + args[1] + "&e."));
          p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&eNote&7: " + message.replaceAll("&", "§")));
