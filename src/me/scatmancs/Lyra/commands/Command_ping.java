  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] arguments)
  {
    ...
    if (arguments.length == 0) {
      if ((sender instanceof Player)) {
        sender.sendMessage(new ColorHandler().translate(new StringBuilder().append("&a").append(sender.getName()).append("'s ping is ").append(((CraftPlayer)sender).getHandle().ping).toString()) + "ms.");
      } else {
    ...
    if (arguments.length == 1)
    {
      ...
      if (target == null) {
        ...
        sender.sendMessage(new ColorHandler().translate(new StringBuilder().append("&a").append(target.getName()).append("'s ping is ").append(((CraftPlayer)target).getHandle().ping).toString()) + "ms.");
      }
    }
    return true;
  }
