            ...
            if (target.getName().equals(sender.getName())) {
                ...
                player.sendMessage(
                    new ColorHandler().translate(
                        "&cYou can not report other player again for &6"
                        + DurationFormatUtils.formatDurationWords(getMillisecondLeft(player),
                                                                  true,
                                                                  true) + "&c."));
            } else {
                ...
                for (Player staff : Bukkit.getServer().getOnlinePlayers()) {
                    if (staff.hasPermission("lyra.player.staff")) {
                        ...
                        messageClick.then(new ColorHandler().translate("&7[&9&lREPORT&7] &9" + player.getName()
                                                                       + " &ehas reported &9" + target.getName()
                                                                       + " &efor &c" + StringUtils.join(arguments,
                                                                                                        ' ',
                                                                                                        1,
                                                                                                        arguments.length) + "&e."));
                        messageClick.tooltip(new ColorHandler().translate("&aClick to teleport to "
                                                                          + target.getName()));
                ...
                player.sendMessage(new ColorHandler().translate("&eSucessfully reported the player &9"
                                                                + target.getName() + " &efor &c"
                                                                + StringUtils.join(arguments,
                                                                                   ' ',
                                                                                   1,
                                                                                   arguments.length) + "&e, report sent to all online staff members."));
            }
        } else {
