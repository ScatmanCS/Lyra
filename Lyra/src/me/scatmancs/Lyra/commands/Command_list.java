...
public class Command_list implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] arguments) {
        if (arguments.length >= 0) {
            sender.sendMessage(
                new ColorHandler().translate("&6&m-----------------------------------------------------"));
            sender.sendMessage(new ColorHandler().translate("&ePlayers Online&7: &a"
                                                            + Bukkit.getServer().getOnlinePlayers().length + "&7/&a"
                                                            + Bukkit.getServer().getMaxPlayers()));
            sender.sendMessage(
                new ColorHandler().translate(" &6* &7If you need assistance join to our TeamSpeak (ts.lyra.com)"));
            sender.sendMessage(
                new ColorHandler().translate("&6&m-----------------------------------------------------"));
        }
